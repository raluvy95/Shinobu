...
createCommand({
    ...
    execute: async (message, args, guild) => {
        ...
        if (!args[0]) return await message.send({embed: usageEmbed("logs", "set <channel> | remove")})
        let option = args.shift()
        switch (option) {
            case "set":
                if (!args[0]) return await message.send({embed: usageEmbed("logs", "set <channel>")})
                let channel = message.mentionedChannels[0]
                if (!channel) {
                    channel = guild.channels.find(e => e.id == args[0] || e.name == args[0] || e.mention == args[0])
                ...
                if (!await botcache.db.config.has(message.guildID)) await botcache.db.config.create(message.guildID, {logs: channel.id, prefix: ".", muted: ""})
            ...
            case "remove":
                ...
                else await botcache.db.config.update(message.guildID, {logs: ""})
